...
def icc_hpc(theta: float, items: np.ndarray) -> np.ndarray:
    ...
    function for all items in a `np.ndarray` are calculated at once.
...
def inf_hpc(theta: float, items: np.ndarray):
    """Implementation of :py:func:`inf` using :py:mod:`numpy` and :py:mod:`numexpr` in which the information
    value for all items in a `np.ndarray` are calculated at once.
...
def test_info(theta: float, items: np.ndarray) -> float:
    ...
    """
    return np.sum(inf_hpc(theta, items))


def var(theta: float, items: np.ndarray) -> float:
...
def see(theta: float, items: np.ndarray) -> float:
...
def reliability(theta: float, items: np.ndarray):
...
def max_info_hpc(items: np.ndarray) -> float:
    """Implementation of :py:func:`max_info` using :py:mod:`numpy` and :py:mod:`numexpr` in which the theta
    value that maximizes the information function for all items is returned in a `np.ndarray`.
...
def log_likelihood(
    est_theta: float, response_vector: list, administered_items: np.ndarray
) -> float:
    ...
    query = np.where(response_vector, np.log(ps), np.log(1 - ps))
...
def negative_log_likelihood(est_theta: float, *args) -> float:
    ...
    :param administered_items np.ndarray: a numpy array containing the parameters of the answered items
...
def normalize_item_bank(items: np.ndarray) -> np.ndarray:
    ...
    if items.shape[1] == 3:
        items = np.append(items, np.ones((items.shape[0])), axis=1)

    return items


def validate_item_bank(items: np.ndarray, raise_err: bool = False):
    ...
    if type(items) is not np.ndarray:
        raise ValueError('Item matrix is not of type {0}'.format(np.ndarray))
