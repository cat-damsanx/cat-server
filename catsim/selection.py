...
class RandomSelector(Selector):
    ...
    def select(
        ...
        **kwargs
        ...
        if self._replace:
            return np.random.choice(items.shape[0])
        else:
            return np.random.choice(list(set(range(items.shape[0])) - set(administered_items)))
...
class ClusterSelector(Selector):
    ...
    @staticmethod
    def sum_cluster_params(items: np.ndarray, c: list):
        ...
        """
        averages = np.zeros((np.max(c) + 1, 4))
    ...
    @staticmethod
    def avg_cluster_params(items: np.ndarray, c: list):
...
class StratifiedSelector(FiniteSelector):
    ...
    @staticmethod
    @abstractmethod
    def sort_items(items: np.ndarray) -> np.ndarray:
        pass
...
class AStratifiedBBlockingSelector(StratifiedSelector):
    ...
    @staticmethod
    def sort_items(items: np.ndarray) -> np.ndarray:
        return np.lexsort((items[:, 0], items[:, 1]))
...
class MaxInfoBBlockingSelector(StratifiedSelector):
    ...
    def sort_items(items: np.ndarray) -> np.ndarray:
        maxinfo = irt.max_info_hpc(items)
        return np.lexsort((irt.inf_hpc(maxinfo, items), maxinfo))
...
class The54321Selector(FiniteSelector):
    ...
    def select(
        ...
        **kwargs
        ...
        return np.random.choice(organized_items[:-bin_size])
...
class RandomesqueSelector(Selector):
    ...
    def select(
        ...
        **kwargs
        ...
        return np.random.choice(list(organized_items)[:self._bin_size])
...
class IntervalIntegrationSelector(Selector):
    ...
    def __init__(self, interval: float = None):
        super().__init__()
        self._interval = interval if interval is not None else np.inf
